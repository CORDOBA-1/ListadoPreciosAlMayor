        ...
        <table>
            <thead>
                <tr>
                    ...
                    <th style="${data[0].alto ? 'width: 80px;' : ''}">${data[0].alto ? 'Alto' : (data[0].nombre ? '' : 'Precio')}</th>
    ...
    data.forEach((item, index) => {
        tableHTML += `
            <tr class="product-row" data-product-index="${index}" data-product-info='${JSON.stringify(item).replace(/'/g, "&#39;")}'>
                ...
                <td class="${item.alto ? 'alto-column' : ''}"><strong>${item.alto || (item.nombre ? '' : item.precio)}</strong></td>
